...
def create_logger(cfg_file, valid_set_name, output_path, log_path):
    ...
    # model path
    final_output_path = os.path.join(output_path, curDir, cfg_name, '{}'.format(valid_set_name))
    ...
    logging.basicConfig(filename=os.path.join(final_log_path, '{}_{}.log'.format(cfg_name, time_str)),
                        format='%(asctime)-15s %(message)s', level=logging.ERROR)
