import cv2
import numpy as np
from common.utility.image_processing_cv import get_single_patch_sample, get_single_patch_sample_inference, get_single_patch_sample_inference_2
...
class infer_facade_Dataset_2:
    def __init__(self, image_list,patch_width, patch_height, aug_config):
        # gather various dataset
        self.db = list()
        for x_offset,y_offset,im in image_list:
    ...
    def __getitem__(self, index):
        img_patch = get_single_patch_sample_inference_2(
            self.db[index]['image'], self.patch_width, self.patch_height, self.mean, self.std, self.aug_config)
