...
default_config_path = os.path.join(project_folder_path, "experiments/resnet/lr1e-3_x120-90-110_center_b2.yaml")
default_model_path = os.path.join(project_folder_path,"model/resnet18_model_latest.pth.tar")

def parse_args():
    parser = argparse.ArgumentParser(description='window detection')
    # yaml config file
    parser.add_argument('--cfg', default=default_config_path, help='experiment configure file name', type=str)
    parser.add_argument('--model', default=default_model_path, help='/path/to/model, requird when resume/test', type=str)
    ...
    parser.add_argument('--infer', default='.', help='/path/to/image/folder', type=str)
