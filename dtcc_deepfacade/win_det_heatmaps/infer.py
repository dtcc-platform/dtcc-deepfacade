...
import dtcc_deepfacade.win_det_heatmaps._init_paths

# project dependence
from dtcc_deepfacade.win_det_heatmaps.common_pytorch.dataset.all_dataset import *
from dtcc_deepfacade.win_det_heatmaps.common_pytorch.config_pytorch import update_config_from_file, update_config_from_args, \
    s_args, s_config, s_config_file
from dtcc_deepfacade.win_det_heatmaps.common_pytorch.common_loss.balanced_parallel import DataParallelModel
from dtcc_deepfacade.win_det_heatmaps.common_pytorch.net_modules import inferNet2, inferNet

from dtcc_deepfacade.win_det_heatmaps.common_pytorch.blocks.resnet_pose import get_default_network_config, get_pose_net, init_pose_net
from dtcc_deepfacade.win_det_heatmaps.common_pytorch.loss.heatmap import get_default_loss_config, get_merge_func

from dtcc_deepfacade.win_det_heatmaps.core.loader import infer_facade_Dataset_2, infer_facade_Dataset
...
def get_windows_with_score(input_image_list):
    # parsing specific config
    config = copy.deepcopy(s_config)
    config.network = get_default_network_config()
    config.loss = get_default_loss_config()
    ...
    dataset_infer = infer_facade_Dataset_2(input_image_list, config.train.patch_width, config.train.patch_height, config.aug)
    ...
    # train and valid
    logger.info("Test DB size: {}.".format(len(input_image_list)))

    beginT = time.time()
    windows_list_with_score = inferNet2(infer_data_loader, net, merge_hm_flip_func, merge_tag_flip_func, flip_pairs,
             config.train.patch_width, config.train.patch_height, config.loss, config.test)
    endt = time.time() - beginT
    logger.info('Speed: %.3f second per image' % (endt / len(input_image_list)))
